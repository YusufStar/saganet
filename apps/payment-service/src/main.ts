...
async function bootstrap() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    const config = new DocumentBuilder()
      ...
      .setDescription('Payment processing — charge, refund, webhook simulation')
      .setVersion('1.0')
    ...
    logger.info(
      `[payment-service] Swagger UI: http://localhost:${process.env.PAYMENT_SERVICE_PORT ?? 3005}/docs`,
    );
  }
