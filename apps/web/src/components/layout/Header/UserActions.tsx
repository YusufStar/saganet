  ...
  return (
    ...
    >
      <Tooltip content={tooltip} position="bottom">
        <Link href={href}>
          <div className={`flex items-center justify-center w-10 h-10 rounded-md cursor-pointer transition-colors relative ${colorClass}`}>
            {children}
            {badge != null && badge > 0 && (
      ...
      <div className="py-1">
        {items.map((item) => (
          <Link key={item.href} href={item.href}>
            <div className="px-4 py-2.5 text-sm text-text-primary hover:bg-orange-50 hover:text-orange-500 transition-colors">
              {item.label}
            </div>
          </Link>
        ))}
