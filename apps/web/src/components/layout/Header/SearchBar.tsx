...
export function SearchBar() {
  ...
  return (
    <div className="flex-1 max-w-[640px] relative">
      <div
        className={[
          'flex items-stretch rounded-[var(--radius-md)] border-2 transition-colors duration-150',
          'bg-[var(--color-neutral-50)]',
          focused ? 'border-orange-500' : 'border-[var(--color-border)]',
        ].join(' ')}
      >
        {/* Category selector */}
        <div className="relative flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1 px-3 h-full text-xs font-medium text-[var(--color-text-secondary)] bg-[var(--color-neutral-100)] border-r border-[var(--color-border)] hover:bg-[var(--color-neutral-200)] transition-colors whitespace-nowrap rounded-l-[calc(var(--radius-md)-2px)] max-w-[120px] truncate"
          >
                ...
                <button
                  ...
                  className={[
                    ...
                    !selectedId
                      ...
                      : 'text-[var(--color-text-primary)] hover:bg-orange-50 hover:text-orange-500',
                  ].join(' ')}
                >
                  {ALL}
                </button>
        ...
        <input
          ...
          className="flex-1 px-4 py-2.5 text-sm bg-transparent outline-none text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] min-w-0"
        />
      ...
      <Popover open={focused && !catOpen && query.length > 0} minWidth={400} className="w-full">
        <div className="px-4 pt-3 pb-1 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">
          Results
        </div>
